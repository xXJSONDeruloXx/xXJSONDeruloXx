#!/usr/bin/env python3
"""Update README plugin download metrics from Deckbrew and GitHub plugin ZIPs."""
...
GITHUB_PLUGIN_REPOS = {
    "Decky--Framegen": {
        ...
        "repo": "Decky-Framegen",
        "color": "6366f1",
        "asset_tokens": ("deckyframegen",),
        "exclude_tokens": ("assets", "binaries", "dlssenabler"),
    },
    "Decky%20LSFG--VK": {
        "owner": "xXJSONDeruloXx",
        "repo": "decky-lsfg-vk",
...
def is_plugin_zip(asset_name: str, asset_tokens: tuple[str, ...], exclude_tokens: tuple[str, ...]) -> bool:
    if not asset_name.lower().endswith(".zip"):
        return False
    normalized = normalized_asset_name(asset_name)
    return any(token in normalized for token in asset_tokens) and not any(token in normalized for token in exclude_tokens)


def fetch_github_plugin_zip_downloads(owner: str, repo: str, asset_tokens: tuple[str, ...], exclude_tokens: tuple[str, ...]) -> int:
    total = 0
    page = 1
    while True:
        ...
        total += sum(
            ...
            if is_plugin_zip(asset.get("name", ""), asset_tokens, exclude_tokens)
        )
        if len(releases) < 100:
            break
        page += 1
...
def replace_flat_badge(markdown: str, label: str, metric: str, color: str, value: int) -> str:
...
def replace_total_badge(markdown: str, value: int) -> str:
    encoded = badge_count(value)
    pattern = r"(total%20tracked%20plugin%20downloads-)[0-9%2C]+(-22c55e\?style=for-the-badge(?:&[^)]+)?)"
    return re.sub(pattern, rf"\g<1>{encoded}\2", markdown)
...
def main() -> None:
    ...
    github = {
        label: fetch_github_plugin_zip_downloads(
            meta["owner"],
            meta["repo"],
            meta["asset_tokens"],
            meta["exclude_tokens"],
        )
        for label, meta in GITHUB_PLUGIN_REPOS.items()
    }

    markdown = README.read_text()

    for plugin_id, meta in DECKY_PLUGINS.items():
        markdown = replace_flat_badge(markdown, meta["label"], "Decky%20Store%20downloads", meta["color"], deckbrew[plugin_id])

    for label, meta in GITHUB_PLUGIN_REPOS.items():
        markdown = replace_flat_badge(markdown, label, "GitHub%20plugin%20ZIP%20downloads", meta["color"], github[label])

    markdown = replace_total_badge(markdown, sum(deckbrew.values()) + sum(github.values()))
